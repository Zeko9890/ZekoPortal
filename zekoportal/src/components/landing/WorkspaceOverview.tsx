...
const statusColor: Record<string, string> = {
  ...
  "Pending invite": "bg-muted text-muted-foreground",
};
  ...
  return (
    <section id="features" className="py-24 bg-muted border-y border-border relative overflow-hidden transition-colors duration-500">
      ...
      <div ref={revealRef} className="reveal max-w-[1100px] mx-auto px-6 md:px-10 relative z-10">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[6px] text-[11px] font-semibold transition-all ${
                  isActive
                    ? "text-foreground border border-border bg-card shadow-sm"
        ...
        {/* Dashboard panel */}
        <div className="rounded-xl border border-border bg-card overflow-hidden shadow-[0_24px_48px_-12px_rgba(0,0,0,0.05)]">

          {/* Panel header */}
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border bg-muted/50">
            <div className="flex items-center gap-2">
              <currentTab.icon className="h-3.5 w-3.5" style={{ color: currentTab.accent }} />
              <span className="text-[11px] font-semibold text-foreground">{currentTab.label}</span>
            </div>
            <div className="flex items-center gap-2">
              <button className="text-[9px] font-medium text-muted-foreground hover:text-foreground px-2 py-1 rounded-[4px] border border-border transition-colors bg-card shadow-sm">
                Filter
              </button>
              <button className="text-[9px] font-medium text-muted-foreground hover:text-foreground px-2 py-1 rounded-[4px] border border-border transition-colors bg-card shadow-sm">
                Sort
          ...
          <div className="px-4 py-2 border-b border-border bg-card">
            {activeTab === "tasks" && (
              <div className="grid grid-cols-[2fr_1.2fr_80px_80px_80px_32px] gap-3 text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                <span>Task</span><span>Project</span><span>Status</span><span>Priority</span><span>Due</span><span></span>
              </div>
            )}
            {activeTab === "projects" && (
              <div className="grid grid-cols-[2fr_1.2fr_100px_80px_100px] gap-3 text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                <span>Project</span><span>Client</span><span>Progress</span><span>Status</span><span>Health</span>
              </div>
            )}
            {activeTab === "clients" && (
              <div className="grid grid-cols-[1.5fr_1.5fr_80px_100px_80px] gap-3 text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                <span>Client</span><span>Company</span><span>Projects</span><span>Portal</span><span>Last seen</span>
              </div>
            )}
            {activeTab === "approvals" && (
              <div className="grid grid-cols-[2fr_1.2fr_1fr_90px_80px] gap-3 text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                <span>Item</span><span>Requested by</span><span>Client</span><span>Status</span><span>Due</span>
              </div>
            )}
            {activeTab === "files" && (
              <div className="grid grid-cols-[2fr_80px_80px_80px_80px_50px] gap-3 text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
          ...
          <div className="divide-y divide-gray-100">
            {activeTab === "tasks" && taskRows.map((row, i) => (
              <div key={i} className="grid grid-cols-[2fr_1.2fr_80px_80px_80px_32px] gap-3 px-4 py-2.5 hover:bg-muted/50 transition-colors items-center group">
                <div className="flex items-center gap-2">
                  <Circle className="h-2.5 w-2.5 text-gray-300 shrink-0" />
                  <span className="text-[10px] font-medium text-foreground truncate">{row.name}</span>
                </div>
                <span className="text-[9px] text-muted-foreground truncate">{row.project}</span>
                <span className={`text-[8px] font-semibold px-1.5 py-0.5 rounded-[3px] w-fit ${statusColor[row.status]}`}>{row.status}</span>
                <span className={`text-[8px] font-semibold ${row.priority === "High" ? "text-red-500" : row.priority === "Medium" ? "text-orange-500" : "text-muted-foreground"}`}>{row.priority}</span>
                <span className="text-[9px] text-muted-foreground font-medium">{row.due}</span>
            ...
            {activeTab === "projects" && projectRows.map((row, i) => (
              <div key={i} className="grid grid-cols-[2fr_1.2fr_100px_80px_100px] gap-3 px-4 py-2.5 hover:bg-muted/50 transition-colors items-center">
                <div className="flex items-center gap-2">
                  <FolderOpen className="h-3 w-3 text-muted-foreground shrink-0" />
                  <span className="text-[10px] font-medium text-foreground truncate">{row.name}</span>
                </div>
                <span className="text-[9px] text-muted-foreground truncate">{row.client}</span>
                <div className="flex items-center gap-1.5">
                  <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div className="h-full bg-sky-500 rounded-full" style={{ width: `${Math.round((row.done / row.tasks) * 100)}%` }} />
                  </div>
                  <span className="text-[8px] text-muted-foreground font-mono w-8 shrink-0">{row.done}/{row.tasks}</span>
                </div>
                <span className={`text-[8px] font-semibold px-1.5 py-0.5 rounded-[3px] w-fit ${statusColor[row.status]}`}>{row.status}</span>
            ...
            {activeTab === "clients" && clientRows.map((row, i) => (
              <div key={i} className="grid grid-cols-[1.5fr_1.5fr_80px_100px_80px] gap-3 px-4 py-2.5 hover:bg-muted/50 transition-colors items-center">
                <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] text-muted-foreground">{row.company}</span>
                <span className="text-[9px] text-muted-foreground font-medium">{row.projects} project{row.projects > 1 ? "s" : ""}</span>
                <span className={`text-[8px] font-semibold px-1.5 py-0.5 rounded-[3px] w-fit ${statusColor[row.portal] || "bg-muted text-muted-foreground"}`}>{row.portal}</span>
                <span className="text-[9px] text-muted-foreground">{row.lastSeen}</span>
              </div>
            ))}

            {activeTab === "approvals" && approvalRows.map((row, i) => (
              <div key={i} className="grid grid-cols-[2fr_1.2fr_1fr_90px_80px] gap-3 px-4 py-2.5 hover:bg-muted/50 transition-colors items-center">
                <div className="flex items-center gap-2">
                  <ThumbsUp className="h-3 w-3 text-muted-foreground shrink-0" />
                  <span className="text-[10px] font-medium text-foreground truncate">{row.item}</span>
                </div>
                <span className="text-[9px] text-muted-foreground truncate">{row.requestedBy}</span>
                <span className="text-[9px] text-muted-foreground truncate">{row.client}</span>
                <span className={`text-[8px] font-semibold px-1.5 py-0.5 rounded-[3px] w-fit ${statusColor[row.status]}`}>{row.status}</span>
                <span className="text-[9px] text-muted-foreground font-medium">{row.due}</span>
              </div>
            ...
            {activeTab === "files" && fileRows.map((row, i) => (
              <div key={i} className="grid grid-cols-[2fr_80px_80px_80px_80px_50px] gap-3 px-4 py-2.5 hover:bg-muted/50 transition-colors items-center">
                <div className="flex items-center gap-2">
                  <Paperclip className="h-3 w-3 text-muted-foreground shrink-0" />
                  <span className="text-[10px] font-medium text-foreground truncate">{row.name}</span>
                </div>
                <span className="text-[9px] text-muted-foreground">{row.type}</span>
                <span className="text-[9px] text-muted-foreground font-mono">{row.size}</span>
                <span className="text-[9px] text-muted-foreground">{row.updatedBy}</span>
                <span className="text-[9px] text-muted-foreground">{row.updated}</span>
                <span className="text-[8px] font-semibold text-muted-foreground bg-muted px-1.5 py-0.5 rounded-[3px] w-fit">{row.version}</span>
              </div>
            ))}
          </div>
          ...
          <div className="px-4 py-2 border-t border-border flex items-center justify-between">
            ...
            <button className="flex items-center gap-1 text-[9px] text-muted-foreground hover:text-foreground transition-colors font-medium">
